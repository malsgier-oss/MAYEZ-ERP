...
export default function Cart() {
  ...
  const handleClear = () => {
    if (items.length > 0 && !window.confirm(t('pos.clear_cart_confirm'))) return
    clearCart()
  }
    ...
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col h-full min-h-[300px]">
      ...
      {customerName && (
        <div className="px-4 py-2 bg-slate-50 text-slate-600 text-sm">
          {t('pos.customer')}: <span className="font-medium">{customerName}</span>
        </div>
      )}
      <div className="flex-1 overflow-auto p-4 space-y-2">
        {items.length === 0 ? (
          <p className="text-slate-500 text-center py-8">{t('pos.cart_empty')}</p>
        ) : (
          items.map((item) => (
            <div
              key={item.productId}
